...
export function newCaroGame(playFirst) {
    const emptyBoard = Array.from({ length: boardSize }, () => Array(boardSize).fill(null));
    return {
        board: emptyBoard,
        turn: 1,
        turnOf: playFirst,
...
export function playCaroTurn(state, side, x, y) {
    if (state.status !== "playing")
        return false;
    if (side !== state.turnOf)
        return false;
...
function checkWinAt(board, x, y, side) {
    const directions = [
        [0, 1], // ngang
        [1, 0], // dọc
        [1, 1], // chéo xuống
